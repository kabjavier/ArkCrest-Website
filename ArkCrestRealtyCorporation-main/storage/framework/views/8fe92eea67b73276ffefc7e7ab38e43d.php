...
<head>
    ...
    <link rel="icon" type="image/png" href="<?php echo e(asset('images/ArkCrest_Logo.png')); ?>">
    <meta name="csrf-token" content="<?php echo e(csrf_token()); ?>">
    <title><?php echo e(config('app.name', 'ARCKREST REALTY CORPORATION')); ?> - <?php echo $__env->yieldContent('title', 'Dashboard'); ?></title>
    <link rel="stylesheet" href="<?php echo e(asset('css/dashboard.css')); ?>?v=<?php echo e(time()); ?><?php echo e(rand(1000, 9999)); ?>">
    <link rel="stylesheet" href="<?php echo e(asset('css/optimized-global.css')); ?>?v=<?php echo e(time()); ?>">
    <link rel="stylesheet" href="<?php echo e(asset('css/mobile-responsive.css')); ?>?v=<?php echo e(time()); ?>">
    <style>
...
<body>
    <div class="dashboard-container">
        ...
        <header class="header">
            <div class="header-content">
                ...
                <div class="logo">
                    <img src="<?php echo e(asset('images/ArkCrest_Logo.png')); ?>" alt="ARCKREST Logo" class="logo-img">
    </div>
                ...
                <div class="header-right">
                    ...
                    <div class="notes-container" style="position:relative;">
                        <button id="notesToggle" class="hdr-btn" title="My Notes">
                            ...
                            <?php if(isset($userNotes) && $userNotes->count() > 0): ?>
                            <span class="hdr-badge hdr-badge-gold"><?php echo e($userNotes->count() > 9 ? '9+' : $userNotes->count()); ?></span>
                            <?php endif; ?>
                        </button>
                        <div id="notesPanel" style="display:none;position:absolute;top:calc(100% + 10px);right:0;width:320px;background:white;border-radius:10px;box-shadow:0 4px 20px rgba(30,69,117,.15);border:1.5px solid #e2e8f0;z-index:9999;overflow:hidden;">
                            <div style="background:linear-gradient(135deg,#1e4575,#2563eb);padding:12px 16px;display:flex;align-items:center;justify-content:space-between;">
                                ...
                                <span id="notesCount" style="color:rgba(255,255,255,.8);font-size:10px;background:rgba(255,255,255,.2);padding:2px 8px;border-radius:20px;font-weight:600;"><?php echo e(isset($userNotes) ? $userNotes->count() : 0); ?> note(s)</span>
                            </div>
                            ...
                            <div style="padding:12px 14px;border-bottom:1px solid #f1f5f9;background:#fafbfc;">
                                <form method="POST" action="<?php echo e(route('notes.store')); ?>">
                                    ...
                                    <div style="display:flex;gap:6px;align-items:center;margin-bottom:6px;">
                                        <input type="date" name="note_date" style="flex:1;padding:6px 8px;border:1.5px solid #e2e8f0;border-radius:7px;font-size:11px;outline:none;" value="<?php echo e(date('Y-m-d')); ?>">
                            ...
                            <div style="max-height:220px;overflow-y:auto;">
                                <?php if(isset($userNotes) && $userNotes->count() > 0): ?>
                                    ...
                                    <div data-note-id="<?php echo e($note->id); ?>" style="padding:10px 14px;border-bottom:1px solid #f8fafc;display:flex;gap:8px;align-items:flex-start;">
                                        <div style="flex:1;min-width:0;">
                                            <div style="font-size:12px;font-weight:600;color:#0f172a;margin-bottom:1px;"><?php echo e($note->title); ?></div>
                                            <?php if($note->body): ?>
                                            <div style="font-size:11px;color:#64748b;line-height:1.4;margin-bottom:3px;"><?php echo e(Str::limit($note->body, 60)); ?></div>
                                            <?php endif; ?>
                                            <?php if($note->note_date): ?>
                                            <div style="font-size:10px;color:#94a3b8;display:flex;align-items:center;gap:3px;margin-top:2px;">
                                                ...
                                                <?php echo e(\Carbon\Carbon::parse($note->note_date)->format('M d, Y')); ?>

                                                <?php if($note->reminder_time): ?>
                                                &bull; <?php echo e(\Carbon\Carbon::parse($note->reminder_time)->format('g:i A')); ?>

                                                <?php endif; ?>
                                            </div>
                                            <?php endif; ?>
                                        </div>
                                        <form method="POST" action="<?php echo e(route('notes.destroy', $note->id)); ?>" style="flex-shrink:0;">
                    ...
                    <div class="notification-container">
                        <button id="notificationToggle" class="hdr-btn" title="Notifications">
                            ...
                            <span class="hdr-badge"><?php echo e($unreadNotifCount > 9 ? '9+' : $unreadNotifCount); ?></span>
                            <?php endif; ?>
                        </button>
                        <div id="notificationPanel" class="notification-panel">
                            <div class="notification-header">
                                ...
                                <span class="notification-count"><?php echo e(isset($unreadNotifCount) && $unreadNotifCount > 0 ? $unreadNotifCount.' unread' : 'All read'); ?></span>
                            </div>
                            <div class="notification-list">
                                <?php if(isset($sysNotifs) && $sysNotifs->count() > 0): ?>
                                    <?php $__currentLoopData = $sysNotifs; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $notif): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                    <div class="notification-item <?php echo e($notif->is_read ? '' : 'unread'); ?>"
                                        style="cursor:<?php echo e(in_array($notif->type, ['note_reminder','user_pending','permission_request','commission_reminder','downpayment_reminder','tripping_reminder']) ? 'pointer' : 'default'); ?>;<?php echo e($notif->is_read && !in_array($notif->type, ['user_pending','permission_request','note_reminder','commission_reminder','downpayment_reminder','tripping_reminder']) ? 'opacity:0.5;pointer-events:none;' : ''); ?>"
                                        <?php if($notif->type === 'note_reminder'): ?>
                                        onclick="event.stopPropagation();openNoteModal(<?php echo e($notif->note_id ?? 0); ?>, '<?php echo e(addslashes($notif->title)); ?>', '<?php echo e(addslashes($notif->message)); ?>', this, <?php echo e($notif->id); ?>)"
                                        <?php elseif($notif->type === 'commission_reminder'): ?>
                                        onclick="event.stopPropagation();window.location='<?php echo e(route('commission-monitoring')); ?>'"
                                        <?php elseif($notif->type === 'downpayment_reminder'): ?>
                                        onclick="event.stopPropagation();window.location='<?php echo e(route('commission-monitoring')); ?>'"
                                        <?php elseif($notif->type === 'tripping_reminder'): ?>
                                        onclick="event.stopPropagation();window.location='<?php echo e(route('site-visit-database')); ?>'"
                                        <?php elseif($notif->type === 'user_pending'): ?>
                                        onclick="event.stopPropagation();window.location='<?php echo e(route('settings')); ?>?panel=users'"
                                        <?php elseif($notif->type === 'permission_request' && auth()->user()->isAdmin()): ?>
                                        onclick="event.stopPropagation();window.location='<?php echo e(route('settings')); ?>?panel=permission-requests'"
                                        <?php elseif(in_array($notif->type, ['permission_approved','permission_rejected','permission_sent'])): ?>
                                        onclick="event.stopPropagation();handlePermissionNotifClick(<?php echo e($notif->id); ?>, '<?php echo e($notif->note_id); ?>')"
                                        <?php endif; ?>>
                                        <div class="notification-icon <?php echo e(in_array($notif->type, ['permission_approved']) ? 'green' : (in_array($notif->type, ['permission_rejected']) ? 'red' : 'blue')); ?>"
                                             style="<?php echo e($notif->type === 'permission_approved' ? 'background:#dcfce7;' : ($notif->type === 'permission_rejected' ? 'background:#fee2e2;' : '')); ?>">
                                            <?php if($notif->type === 'note_reminder'): ?>
                                            <svg fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg>
                                            <?php elseif($notif->type === 'user_pending'): ?>
                                            <svg fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"/></svg>
                                            <?php elseif(in_array($notif->type, ['permission_request','permission_sent'])): ?>
                                            <svg fill="none" stroke="<?php echo e($notif->type === 'permission_request' ? '#1d4ed8' : '#1e4575'); ?>" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 7a2 2 0 012 2m4 0a6 6 0 01-7.743 5.743L11 17H9v2H7v2H4a1 1 0 01-1-1v-2.586a1 1 0 01.293-.707l5.964-5.964A6 6 0 1121 9z"/></svg>
                                            <?php elseif($notif->type === 'permission_approved'): ?>
                                            ...
                                            <?php elseif($notif->type === 'permission_rejected'): ?>
                                        ...
                                        <div class="notification-content">
                                            ...
                                            <p class="notification-text"><?php echo e($notif->message); ?></p>
                                            <p class="notification-time"><?php echo e($notif->notified_at->diffForHumans()); ?></p>
                                        </div>
                                        <?php if(!$notif->is_read): ?><span class="notif-dot"></span><?php endif; ?>
                                    </div>
                                    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                                <?php else: ?>
        ...
        <div class="content-wrapper">
                ...
                <div class="profile-content">
                    <div class="profile-avatar">
                        <?php if(auth()->user()->avatar): ?>
                            <img src="<?php echo e(str_starts_with(auth()->user()->avatar, 'avatars/') ? \Storage::disk('public')->url(auth()->user()->avatar) : asset(auth()->user()->avatar)); ?>" alt="avatar" style="width:100%;height:100%;object-fit:cover;border-radius:50%;">
                        <?php else: ?>
                            <span><?php echo e(substr(auth()->user()->name ?? 'U', 0, 1)); ?></span>
                        <?php endif; ?>
                    </div>
                    <div class="sidebar-text profile-info">
                        <h3 class="profile-name"><?php echo e(auth()->user()->name ?? 'User Name'); ?></h3>
                        <p class="profile-email"><?php echo e(auth()->user()->email ?? ''); ?></p>
                        <span class="profile-role-badge"><?php echo e(auth()->user() ? ucfirst(auth()->user()->role) : 'Staff'); ?></span>
                    </div>
                </div>
            ...
            <nav class="nav-menu">
                ...
                <ul class="nav-list">
                    ...
                    ?>
                    <?php if(count($financeChildren) > 0): ?>
                    <li class="nav-item-wrapper">
                        <div class="nav-item-container">
                            <a href="<?php echo e(route('dashboard')); ?>" class="nav-item nav-item-with-dropdown" data-page="dashboard" onclick="event.stopPropagation();">
                        ...
                        <ul class="nav-submenu" id="financeSubmenu">
                            <?php if($canSee('departments')): ?>
                            <li>
                                <a href="<?php echo e(route('departments.admin')); ?>" class="nav-subitem" data-page="departments">
                                    <svg class="nav-icon" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            ...
                            <?php endif; ?>
                            <?php if($canSee('summary-report')): ?>
                            <li>
                                <a href="<?php echo e(route('summary-report')); ?>" class="nav-subitem" data-page="summary-report">
                                    <svg class="nav-icon" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            ...
                            <?php endif; ?>
                            <li>
                                <a href="<?php echo e(route('arkcrest-sales')); ?>" class="nav-subitem" data-page="arkcrest-sales">
                                    <svg class="nav-icon" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/>
                                    </svg>
                            ...
                            <?php if($canSee('commission-monitoring')): ?>
                            <li class="nav-item-wrapper">
                                <div class="nav-item-container">
                                    <a href="<?php echo e(route('commission-monitoring')); ?>" class="nav-subitem nav-item-with-dropdown" data-page="commission-monitoring" onclick="event.stopPropagation();">
                                        <svg class="nav-icon" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                ...
                                <ul class="nav-submenu" id="commissionSubmenu" style="padding-left:12px;">
                                    <li>
                                        <a href="<?php echo e(route('commission-dashboard')); ?>" class="nav-subitem" data-page="commission-dashboard" style="font-size:11px;">
                            ...
                            <?php endif; ?>
                            <?php if($canSee('calendar')): ?>
                            <li>
                                <a href="<?php echo e(route('calendar')); ?>" class="nav-subitem" data-page="calendar">
                                    <svg class="nav-icon" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <?php if($canSee('sales-marketing')): ?>
                    <li class="nav-item-wrapper">
                        <div class="nav-item-container">
                            <a href="<?php echo e(route('sales-marketing')); ?>" class="nav-item nav-item-with-dropdown" data-page="sales-marketing" onclick="event.stopPropagation();">
                                <svg class="nav-icon" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        <ul class="nav-submenu" id="salesSubmenu">
                            <li class="nav-item-wrapper">
                                <div class="nav-item-container">
                                    <a href="<?php echo e(route('client-database')); ?>" class="nav-subitem nav-item-with-dropdown" data-page="client-database" onclick="event.stopPropagation();">
                                        <svg class="nav-icon" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                ...
                                <ul class="nav-submenu" id="clientDbSubmenu" style="padding-left:12px;">
                                    <li>
                                        <a href="<?php echo e(route('reserved-clients')); ?>" class="nav-subitem" data-page="cd-clients" style="font-size:11px;">
                                    ...
                                    <li>
                                        <a href="<?php echo e(route('property-list')); ?>" class="nav-subitem" data-page="cd-properties" style="font-size:11px;">
                            ...
                            <li>
                                <a href="<?php echo e(route('site-visit-database')); ?>" class="nav-subitem" data-page="site-visit-database">
                                    <svg class="nav-icon" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"/>
                                    </svg>
                            ...
                            <li>
                                <a href="<?php echo e(route('sales-calendar')); ?>" class="nav-subitem" data-page="sm-calendar">
                                    <svg class="nav-icon" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"/>
                                    </svg>
                                    <span class="sidebar-text">Calendar</span>
                    ...
                    <!-- Human Resource -->
                    <?php if($canSee('human-resource')): ?>
                    <li class="nav-item-wrapper">
                        <div class="nav-item-container">
                            <a href="<?php echo e(route('human-resource')); ?>" class="nav-item nav-item-with-dropdown" data-page="human-resource" onclick="event.stopPropagation();">
                                <svg class="nav-icon" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0z"/>
                                </svg>
                        ...
                        <ul class="nav-submenu" id="hrSubmenu">
                            <li>
                                <a href="<?php echo e(route('hr.employee-data')); ?>" class="nav-subitem" data-page="hr-employee-data">
                                    <svg class="nav-icon" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            ...
                            <li>
                                <a href="<?php echo e(route('hr.contact-list')); ?>" class="nav-subitem" data-page="hr-contact-list">
                                    <svg class="nav-icon" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <li class="nav-item-wrapper">
                        <div class="nav-item-container">
                            <a href="<?php echo e(route('forms')); ?>" class="nav-item nav-item-with-dropdown" data-page="forms" onclick="event.stopPropagation();">
                                <svg class="nav-icon" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        <ul class="nav-submenu" id="formsSubmenu">
                            <li>
                                <a href="<?php echo e(route('forms')); ?>?tab=budget" class="nav-subitem" data-page="forms-budget">
                                    <svg class="nav-icon" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/>
                                    </svg>
                            ...
                            <li>
                                <a href="<?php echo e(route('forms')); ?>?tab=site-visit" class="nav-subitem" data-page="forms-site-visit">
                                    <svg class="nav-icon" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <li class="nav-item-wrapper">
                        <div class="nav-item-container">
                            <a href="<?php echo e(route('settings')); ?>" class="nav-item nav-item-with-dropdown" data-page="settings" onclick="event.stopPropagation();">
                                <svg class="nav-icon" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        <ul class="nav-submenu" id="settingsSubmenu">
                            ...
                            <li>
                                <a href="<?php echo e(route('settings')); ?>?panel=profile" class="nav-subitem" data-page="settings-profile">
                            ...
                            <li>
                                <a href="<?php echo e(route('settings')); ?>?panel=employee-info" class="nav-subitem" data-page="settings-employee-info">
                                    <svg class="nav-icon" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 6H5a2 2 0 00-2 2v9a2 2 0 002 2h14a2 2 0 002-2V8a2 2 0 00-2-2h-5m-4 0V5a2 2 0 114 0v1m-4 0a2 2 0 104 0m-5 8a2 2 0 100-4 2 2 0 000 4zm0 0c1.306 0 2.417.835 2.83 2M9 14a3.001 3.001 0 00-2.83 2"/></svg>
                                    <span class="sidebar-text">About Me</span>
                                </a>
                            </li>
                            <li>
                                <a href="<?php echo e(route('settings')); ?>?panel=system" class="nav-subitem" data-page="settings-system">
                            ...
                            <li>
                                <a href="<?php echo e(route('settings')); ?>?panel=notes" class="nav-subitem" data-page="settings-notes">
                            ...
                            <li>
                                <a href="<?php echo e(route('settings')); ?>?panel=privacy" class="nav-subitem" data-page="settings-privacy">
                            ...
                            ?>
                            <?php if($isAdminUser || array_filter(['settings.users','settings.visibility','settings.activity','settings.deleted','settings.permissions','settings.teams','settings.period-lock'], fn($k) => !in_array($k, $sHiddenGlobal))): ?>
                            <li class="nav-submenu-label">Admin</li>
                            <?php endif; ?>
                            <?php if($canSeeSetting('settings.users')): ?>
                            <li>
                                <a href="<?php echo e(route('settings')); ?>?panel=users" class="nav-subitem" data-page="settings-users">
                            ...
                            <?php endif; ?>
                            <?php if($isAdminUser || $canSeeSetting('settings.visibility')): ?>
                            <li>
                                <a href="<?php echo e(route('settings')); ?>?panel=visibility" class="nav-subitem" data-page="settings-visibility">
                                    <svg class="nav-icon" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/></svg>
                                    <span class="sidebar-text">Page Visibility</span>
                                </a>
                            </li>
                            <?php endif; ?>
                            <?php if($canSeeSetting('settings.activity')): ?>
                            <li>
                                <a href="<?php echo e(route('settings')); ?>?panel=activity" class="nav-subitem" data-page="settings-activity">
                            ...
                            <?php endif; ?>
                            <?php if($canSeeSetting('settings.deleted')): ?>
                            <li>
                                <a href="<?php echo e(route('settings')); ?>?panel=deleted" class="nav-subitem" data-page="settings-deleted">
                            ...
                            <?php endif; ?>
                            <?php if($canSeeSetting('settings.permissions')): ?>
                            <li>
                                <a href="<?php echo e(route('settings')); ?>?panel=permission-requests" class="nav-subitem" data-page="settings-permission-requests">
                                    ...
                                    <?php $pendingPerms = \App\Models\PermissionRequest::where('status','pending')->count(); ?>
                                    <?php if($pendingPerms > 0): ?><span style="background:#ef4444;color:white;border-radius:20px;padding:1px 7px;font-size:10px;font-weight:700;margin-left:auto;"><?php echo e($pendingPerms); ?></span><?php endif; ?>
                                </a>
                            </li>
                            <?php endif; ?>
                            <?php if($canSeeSetting('settings.teams')): ?>
                            <li>
                                <a href="<?php echo e(route('settings')); ?>?panel=teams" class="nav-subitem" data-page="settings-teams">
                            ...
                            <?php if($isAdminUser): ?>
                            <li>
                                <a href="<?php echo e(route('settings')); ?>?panel=properties" class="nav-subitem" data-page="settings-properties">
                                    <svg class="nav-icon" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6"/></svg>
                                    <span class="sidebar-text">Property Management</span>
                                </a>
                            </li>
                            <?php endif; ?>
                            <?php if($canSeeSetting('settings.period-lock')): ?>
                            <li>
                                <a href="<?php echo e(route('settings')); ?>?panel=period-lock" class="nav-subitem" data-page="settings-period-lock">
    ...
    <script>
        ...
        function notifClearAll() {
            showConfirm('This will clear all notifications. This cannot be undone.', function() {
                fetch('<?php echo e(route("notifications.clearAll")); ?>', {
                method: 'POST',
                headers: { 'X-CSRF-TOKEN': '<?php echo e(csrf_token()); ?>', 'Content-Type': 'application/json' }
            }).then(() => {
    ...
    <script src="<?php echo e(asset('js/sidebar-toggle.js')); ?>?v=<?php echo e(time()); ?>"></script>
    <script src="<?php echo e(asset('js/sidebar-active.js')); ?>?v=<?php echo e(time()); ?>"></script>
    <script src="<?php echo e(asset('js/global-search.js')); ?>?v=<?php echo e(time()); ?>"></script>

    <script>
    // ===== REAL-TIME NOTIFICATION POLLING (every 30s) =====
    let _lastUnread = <?php echo e(isset($unreadNotifCount) ? $unreadNotifCount : 0); ?>;
    ...
    *::-webkit-scrollbar { display: none; }
    ...
    <form id="_autoLogoutForm" method="POST" action="<?php echo e(route('logout')); ?>" style="display:none;">
        <?php echo csrf_field(); ?>
    </form>
...
</html><?php /**PATH C:\Users\Joshua\Desktop\ArkCrest github working\ArkCrest-Website\ArkCrest-Website\ArkCrestRealtyCorporation-main\resources\views/layouts/dashboard.blade.php ENDPATH**/ ?>