...
class DatabaseBackupService
{
    ...
    public function listBackups(): array
    {
        ...
        foreach ($files as $path) {
            ...
            if (!preg_match('/\.(xlsx|pdf)$/i', $filename)) continue;

            $backups[] = [
                'filename'   => $filename,
                'type'       => str_ends_with(strtolower($filename), '.pdf') ? 'pdf' : 'xlsx',
    ...
    public function resolveSafePath(string $filename): string
    {
        ...
        if (!preg_match('/^backup_[0-9]{4}-[0-9]{2}-[0-9]{2}_[0-9]{6}\.(xlsx|pdf)$/', $filename)) {
    ...
    /* ------------------------------------------------------------------ *
     |  Excel backup (restorable)
     * ------------------------------------------------------------------ */

    /**
     * Export every backupable table into its own sheet in a single .xlsx workbook.
     * Returns the generated filename.
     */
    public function createExcelBackup(): string
    {
        ...
        foreach ($tables as $index => $table) {
            ...
            $sheetName = substr(preg_replace('/[:\\\\\/\?\*\[\]]/', '_', $table), 0, 31);
            $sheet->setTitle($sheetName);

            // Header row
        
            foreach ($columns as $colIndex => $colName) {
                $sheet->setCellValue([$colIndex + 1, 1], $colName);
            }

            // Data rows, chunked to keep memory usage sane
            $rowNum = 2;
            DB::table($table)->orderBy($columns[0])->chunk(self::CHUNK_SIZE, function ($rows) use ($sheet, $columns, &$rowNum) {
                foreach ($rows as $row) {
                    $rowArray = (array) $row;
                    foreach ($columns as $colIndex => $colName) {
                        $value = $rowArray[$colName] ?? null;
                        // Store everything as a string to avoid PhpSpreadsheet type-guessing issues
                        // with booleans, JSON blobs, or binary-looking data.
                        // 
                        $sheet->getCell([$colIndex + 1, $rowNum])->setValueExplicit(
        ...
        // If there were zero tables (shouldn't happen), make sure workbook isn't empty
    ...
    public function createPdfBackup(): string
    {
        ...
        foreach ($tables as $table) {
            ...
            if (DB::table($table)->count() > 1000) {
                $html .= '<div class="empty-note">Showing first 1000 rows only. Use the Excel backup for a complete export.</div>';
            }
        }
    ...
    /**
     * Restore the database from a previously created (or freshly uploaded) Excel backup.
     *
     * For every sheet whose name matches a real, currently-existing table:
     *   - the table is truncated
     *   - rows from the sheet are re-inserted, matched by column header name
     ...
     *
     * Sheets that don't correspond to a real table are skipped and reported
     ...
     */
    public function restoreFromExcel(string $absoluteFilePath): array
    {
        ...
        try {
            ...
            foreach ($spreadsheet->getSheetNames() as $sheetName) {
                ...
                if (in_array($sheetName, self::EXCLUDED_TABLES, true)) {
                    $skipped[] = $sheetName;
                    continue;
                }
                ...
                if ($highestRow < 1) {
                    $skipped[] = $sheetName . ' (empty)';
                    continue;
                }

                // Header row -> file column names
                $fileColumns = [];
                for ($c = 1; $c <= $highestColIndex; $c++) {
                    // 
                    $fileColumns[$c] = (string) $sheet->getCell([$c, 1])->getValue();
                }

                $tableColumns = Schema::getColumnListing($sheetName);

                DB::table($sheetName)->truncate();

                $batch = [];
                for ($r = 2; $r <= $highestRow; $r++) {
                    $record = [];
                    foreach ($fileColumns as $c => $colName) {
                    ...
                    if (count($batch) >= self::CHUNK_SIZE) {
                        DB::table($sheetName)->insert($batch);
                        $batch = [];
                    }
                }
                if (!empty($batch)) {
